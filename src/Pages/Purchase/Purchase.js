...
const Purchase = () => {
  ...
  const url = `https://wooden-tools.onrender.com/handTools/${purchaseId}`;
  const { data: handTool, isLoading } = useQuery(["tools", purchaseId], () =>
  ...
  const onSubmit = (data) => {
    ...
    fetch("https://wooden-tools.onrender.com/order", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="container mx-auto mt-28">
      <h2 className="text-4xl mb-10 text-secondary text-center font-bold">
        Purchase Page
      </h2>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          <h3 className="text-xl text-secondary font-semibold mb-2">
            Factory directly sales{" "}
            <span className="text-primary text-2xl">
              {" "}
              {handTool.productName}
            </span>
          </h3>
          <hr className="my-2" />
          ...
          <h4 className="text-3xl text-primary mb-4">
            Price: ${handTool.perPrice}/
            <span className="text-lg text-secondary">Per piece</span>
          </h4>
          <hr className="mb-4" />
          <p className="text-xl font-semibold">
            Quantity: {handTool.minQuantity}-{handTool.maxQuantity}{" "}
            <span className="text-sm text-secondary">Pieces</span>
          </p>
          <hr className="my-2" />

          <h3 className="text-xl mb-4 text-primary font-bold text-center   ">
            Manage Your Quantity
          </h3>
          <div className="text-center mb-8 flex">
            <button
              onClick={handleDecrease}
              className="btn-md btn rounded-r-none"
            >
              <FontAwesomeIcon icon={faMinus} />
            </button>
            <input
              ref={inputRef}
              className="border w-3/4 text-center input-bordered input rounded-none text-lg"
              placeholder={handTool.minQuantity}
              value={handTool.minQuantity}
            />
            <button
              onClick={handleIncrease}
              className="btn-md btn rounded-l-none"
            >
        ...
        <div>
          ...
          <div className="w-full text-center">
            ...
            <button className="btn btn-primary btn-sm rounded-full bg-orange-600 text-white w-1/2 my-4">
              <FontAwesomeIcon
                className="mr-2 text-lg font-normal"
                icon={faEnvelope}
              />
              Contact Supplier
            </button>
